"""
Tools for the Agentra Researcher agent
"""
import os
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            AGENTRA_API_ENDPOINT,
            json=document,
            headers={"x-api-key": AGENTRA_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the Agentra knowledge base.
    ...
    """
    if not AGENTRA_API_ENDPOINT or not AGENTRA_API_KEY:
        return {
            "success": False,
            "error": "Agentra API not configured. Running in local mode."
        }
