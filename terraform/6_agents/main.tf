...
# SQS for Asyncronous job processing
...
# AI Agents Lambda roles, policy permissions
...
# Upload shared dependencies layer to S3
resource "aws_s3_object" "shared_layer" {
  bucket = aws_s3_bucket.lambda_packages.id
  key    = "layers/shared_layer.zip"
  source = "${path.module}/../../backend/shared_layer.zip"
  etag   = fileexists("${path.module}/../../backend/shared_layer.zip") ? filemd5("${path.module}/../../backend/shared_layer.zip") : null
}

# Lambda Layer for shared dependencies (litellm, openai-agents, langfuse, boto3, pydantic)
...
# Upload slim handler zips to S3 (deps are in the shared layer)
...
resource "aws_lambda_function" "reporter" {
  function_name = "agentra-reporter"
  role          = aws_iam_role.lambda_agents_role.arn
...
resource "aws_lambda_function" "retirement" {
  function_name = "agentra-retirement"
  role          = aws_iam_role.lambda_agents_role.arn
...
resource "aws_cloudwatch_log_group" "agent_logs" {
  ...
  name              = "/aws/lambda/agentra-${each.key}"
  retention_in_days = 7
